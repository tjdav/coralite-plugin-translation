...
import { parseTranslatedPayload, restoreAttributes } from './block-utils.js'
import { validateChunk } from './validation.js'
import { reconstructHtml, TranslationMarkupError } from './extraction-utils.js'
...
export async function processBlockBatch (lang, payload, mapping, systemPrompt, retries, options) {
  try {
    ...
    for (const item of mapping) {
      ...
      let translatedHtml = translations.get(item.index)

      // Reconstruct Phase 3:
      // We pass the parsed internalMap that should be attached to the mapping item in Phase 1
      if (item.internalMap) {
        translatedHtml = reconstructHtml(translatedHtml, item.internalMap)
      }

      // Attempt to restore attributes
      translatedHtml = restoreAttributes(item.originalHtml, translatedHtml)
      translations.set(item.index, translatedHtml)
    ...
    if (retries > 0) {
      ...
      if (error instanceof TranslationMarkupError) {
        updatedSystemPrompt += `\n\nERROR IN PREVIOUS ATTEMPT: ${error.message}`
      }
      return await processBlockBatch(lang, payload, mapping, updatedSystemPrompt, retries - 1, options)
    }
    throw error
  }
}
