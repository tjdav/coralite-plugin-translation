 ...
 * @param {string} [options.baseURL] - The base URL for canonical and hreflang links.
 * @param {string} [options.cacheDir='.coralite/translations'] - Directory to store translation cache.
...
export function translation (options) {
  ...
  const cacheDir = options.cacheDir || join(process.cwd(), '.coralite', 'translations')
  const cacheFile = join(cacheDir, 'i18n.json')
  const usageFile = join(cacheDir, 'i18n-usage.json')
  const exclude = options.exclude || []
  const chunkSize = options.chunkSize || 10
  const maxTokensMultiplier = options.maxTokensMultiplier || 1000
  const retries = options.retries || 3
  let baseURL = options.baseURL ? new URL(options.baseURL).href : undefined
  ...
  async function loadCache () {
    ...
    try {
      ...
      usageCache = JSON.parse(usageContent)
    } catch (error) {
      // Usage file might not exist yet
      usageCache = Object.create(null)
    }
  ...
  async function saveCache () {
    ...
    savePromise = savePromise.then(async () => {
      try {
        ...
        const serialisableUsage = {}
        for (const [page, hashes] of Object.entries(usageCache)) {
          serialisableUsage[page] = Object.keys(hashes)
        }

        await writeFile(usageFile, JSON.stringify(serialisableUsage, null, 2))
      } catch (error) {
  ...
  // Utility function to remove no-translate attribute from the AST
  function removeNoTranslate (node) {
    if (!node) return
    if (node.type === 'tag' && node.attribs && Object.prototype.hasOwnProperty.call(node.attribs, 'no-translate')) {
      delete node.attribs['no-translate']
    }
    if (node.children) {
      for (const child of node.children) {
        removeNoTranslate(child)
      }
    }
  }
  ...
  const plugin = createPlugin({
    name: 'translation',
    async onBeforePageRender ({ document }) {
      const relativePath = relative(this.options.pages, document.path.pathname)

      // Ensure <html> has the correct lang attribute for the source page
      let htmlNode = document.root.children.find(n => n.type === 'tag' && n.name === 'html')
      if (!htmlNode) {
        // Look deeper if not at root
        const findHtml = (node) => {
          if (node.type === 'tag' && node.name === 'html') return node
          if (node.children) {
            for (const child of node.children) {
        ...
        htmlNode = findHtml(document.root)
      }

      if (htmlNode) {
        htmlNode.attribs = htmlNode.attribs || {}
        htmlNode.attribs.lang = options.sourceLanguage
      }

      if (baseURL) {
        let headNode = null
        const findHead = (node) => {
          if (node.type === 'tag' && node.name === 'head') return node
          if (node.children) {
            for (const child of node.children) {
              const result = findHead(child)
              if (result) return result
            }
          }
          return null
        }
        headNode = findHead(document.root)

        if (headNode) {
          // Add hreflang tags
          const pathUrl = relativePath.endsWith('index.html') && relativePath !== 'index.html'
            ? relativePath.replace(/index\.html$/, '')
            : relativePath.replace(/\/index\.html$/, '')

          const formattedPath = pathUrl === 'index.html' ? '' : (pathUrl.startsWith('/') ? pathUrl : `/${pathUrl}`)

          // target languages
          options.targetLanguages.forEach(lang => {
            const langUrl = `${baseURL}/${lang}${formattedPath}`
          ...
          // source language
          const sourceUrl = `${baseURL}${formattedPath}`
          headNode.children.unshift({
            type: 'tag',
            name: 'link',
            attribs: {
              rel: 'alternate',
              hreflang: options.sourceLanguage,
              href: sourceUrl
            },
            children: []
          })
          ...
          // Check if canonical exists, update or create it
          let canonicalNode = headNode.children.find(n => n.type === 'tag' && n.name === 'link' && n.attribs && n.attribs.rel === 'canonical')
          if (canonicalNode) {
            canonicalNode.attribs.href = sourceUrl
          } else {
            headNode.children.unshift({
              type: 'tag',
              name: 'link',
    ...
    async onAfterPageRender (context) {
      const { path, html } = context
      if (!cacheLoaded) {
        await loadCache()
      }
      ...
      // Remove no-translate attribute from source html before it gets written
      const parsedSource = parseHTML(html)
      removeNoTranslate(parsedSource.root)
      context.html = this.transform(parsedSource.root)

      const generatedPages = []

      // Track hashes used on this page
      const pageHashes = {}
      ...
      for (const targetLang of targetLanguages) {
        ...
        if (blocks.length > 0) {
          ...
          for (const block of blocks) {
            ...
            const sourceHtml = useOuter ? serializeNode(block) : serializeNode({ children: block.children })
...
5. If a block contains no translatable comments or strings, return it exactly as it was provided.`
          ...
          // Update usage cache before saving
          usageCache[relativePath] = pageHashes
          ...
          for (const block of blocks) {
            ...
            if (translatedHtml) {
              const useOuter = hasTranslatableAttributes(block)
              reconstructBlock(block, translatedHtml, parseHTML, useOuter)
            } else {
          ...
          if (success) {
            ...
            // Remove no-translate attribute from target document
            removeNoTranslate(parsed.root)

            // Set lang attribute to targetLang
            ...
            const htmlNode = findNode(parsed.root, 'html')
            if (htmlNode) {
              htmlNode.attribs = htmlNode.attribs || {}
              htmlNode.attribs.lang = targetLang
            }

            if (baseURL) {
              const headNode = findNode(parsed.root, 'head')
              if (headNode) {
                const pathUrl = relativePath.endsWith('index.html') && relativePath !== 'index.html'
                  ? relativePath.replace(/index\.html$/, '')
                  : relativePath.replace(/\/index\.html$/, '')
                const formattedPath = pathUrl === 'index.html' ? '' : (pathUrl.startsWith('/') ? pathUrl : `/${pathUrl}`)
                const targetLangUrl = `${baseURL}/${targetLang}${formattedPath}`

                const canonicalNode = headNode.children.find(n => n.type === 'tag' && n.name === 'link' && n.attribs && n.attribs.rel === 'canonical')
                if (canonicalNode) {
                  canonicalNode.attribs.href = targetLangUrl
                }
              }
            }
      ...
      // Update usage cache for this page (ensure it's set even if no blocks found)
      if (!usageCache[relativePath]) {
        usageCache[relativePath] = pageHashes
      }
  ...
  plugin.onPageDelete = async function ({ path }) {
    if (!cacheLoaded) {
      await loadCache()
    }
    const relativePath = relative(this.options.pages, path.pathname)

    // Remove usage entry for the deleted page
    if (usageCache[relativePath]) {
      delete usageCache[relativePath]
    }
  }

  plugin.onAfterBuild = async function (context) {
    if (!cacheLoaded) {
      await loadCache()
    }

    const builtPages = Array.isArray(context.results) ? context.results.length : 0
    const totalPages = this.pages.list.length || 0

    // Only clean up if all pages have been built
    if (builtPages < totalPages) {
      console.log(`[Translation] Skipping cleanup: partial build (${builtPages}/${totalPages} pages processed).`)
      return
    }

    // Consolidate all used hashes from all pages
    const allUsedHashes = {}
    for (const hashes of Object.values(usageCache)) {
      for (const hash of Object.keys(hashes)) {
        allUsedHashes[hash] = true
      }
    }

    // Remove unused translations
    let deletedCount = 0
    for (const hash of Object.keys(fragmentCache)) {
      if (!allUsedHashes[hash]) {
        delete fragmentCache[hash]
        deletedCount++
      }
    }

    if (deletedCount > 0) {
      console.log(`[Translation] Cleaned up ${deletedCount} unused translations.`)
      await saveCache()
    }
  }
