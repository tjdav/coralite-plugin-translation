import { TRANSLATABLE_ATTRS, serializeNode } from './block-utils.js'
...
export function extractHtmlPayload (node, useOuter = true) {
  ...
  function serializeForAi (currentNode, isRoot = false) {
    ...
    if (currentNode.type === 'tag' || currentNode.type === 'script' || currentNode.type === 'style') {
      // Check if it's a no-translate element. If so, treat it entirely as a void tag (opaque block)
      if (currentNode.attribs && Object.prototype.hasOwnProperty.call(currentNode.attribs, 'no-translate')) {
        const id = tagCounter++
        internalMap[id] = {
          open: serializeNode(currentNode),
          type: 'void'
        }
        return `$${id}`
      }
