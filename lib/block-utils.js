...
import { extractHtmlPayload } from './extraction-utils.js'

/**
 * Utility functions for block-level translation.
 */

export const TRANSLATABLE_ATTRS = new Set(['alt', 'title', 'placeholder', 'aria-label'])
...
export function findTranslatableBlocks (node, blocks) {
  ...
  // Skip elements with no-translate attribute (and their children)
...
export function prepareTranslationPayload (blocks) {
  ...
  blocks.forEach((block, index) => {
    ...
    const originalHtml = useOuter ? serializeNode(block) : serializeNode({ children: block.children })

    // Use extraction utility to generate payload and map
    const { payload: aiPayload, map: internalMap } = extractHtmlPayload(useOuter ? block : { children: block.children }, useOuter)
